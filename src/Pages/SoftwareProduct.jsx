...
function WebProduct() {
  ...
  useEffect(() => {
    const productsRef = dbRef(db, 'sassProducts');
      ...
      <div className='pmain'>
        ...
        <div className="product-list">
          <div style={{display: "flex", flexDirection: "1 column"}}>
            <div>      
              <h1 style={{ color:"#FFF"}}>Trending Software </h1>
              <p style={{color: "#fff"}}>Recommended for you</p>
            </div>
            <div><a href='/software'><button style={{marginLeft:"370px", width: "200px"}}>Sell Softwares</button></a></div>
          </div>
          <div className='myproduct'>
          {products.map(product => (
            <div key={product.id} className="product-item">
              <img src={product.imageUrl} alt={product.name} className="product-image" />
              <h3>{product.softwareName}</h3>
