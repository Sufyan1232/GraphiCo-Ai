...
function InstaProduct() {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const productsRef = dbRef(db, 'instaProducts');
      ...
      <div className='pmain'>
        ...
        <div className="product-list">
          <div style={{display: "flex", flexDirection: "1 column"}}>
            <div>      
              <h1 style={{color:"#FFF"}}>Trending InstaGram Account</h1>
              <p style={{color: "#fff"}}>Recommended for you</p>
            </div>
            <div><a href='/insta'><button style={{marginLeft:"370px", width: "200px"}}>Sell Your Insta Account</button></a></div>
          </div>
          ...
          {products.map(product => (
            <div key={product.id} className="product-item">
              ...
              <p>Subscribers: {product.followers}</p>
              <p>Videos: {product.numOfPosts}</p>
